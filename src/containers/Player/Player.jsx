...
import './Player.css';
import TracksList from '../../components/TracksList/TracksList.jsx';
import DB from '../../assets/js/db.json';
import axios from "axios";

let todasCanciones = JSON.parse(localStorage.getItem("canciones"));
console.log(DB)
...
state = {
        ...
        shuffle: false,
        sidebar: false,
        //Reproducir cancion automaticamente
        playing: false,
        //Volumen por default
        volume: 1,
        trackLoaded: false,
    ...
    //Cambiar boton de favorito y actualizar con Axios
    toggleFavourite = () => {
        ...
        this.state.tracksList.map((track) => {
            if (track.id === this.state.trackID) {
                ...
                axios.put(`http://localhost:8080/song/favourite/${this.state.trackID}`, { favourite: !this.state.favourite });
                console.log(this.state.trackID);
                console.log(!this.state.favourite);
                this.state.favourite = !this.state.favourite;
                alert(this.state.favourite ? "Agregado a la lista" : "Eliminado de la lista")
            }
            return track;
        });
    }
    ...
    //Funcion para reproducir y pausar audios (Palanca)
    playPauseAudio = () => {
    ...
    loadTrackIntoState = () => {
        ...
        const trackImg = this.state.tracksList[this.state.currentTrackIndex].img;
        const player = this.state.player;
        // player.pause();
        ...
        this.setState(
            {
                ...
                trackImg: trackImg,
                favourite: this.state.tracksList[this.state.currentTrackIndex].favourite,
                player: player,
                trackLoaded: false,
                playing: false,
    ...
    hola = () => {
        console.log(this.state.trackID);
        alert("Se ha agregado a tu lista de MG")
    }
    ...
    render() {
        return (
            ...
            <div className="Player">
                ...
                <div className="columns-ctr">

                    <img className="track-img" src={this.state.trackImg}></img>

                    <div className="column-ctr-column">
                        ...
                        <div className="ctr-1">
                            <p className="play-pause-text">
                                <strong>
                                    {this.state.playing ? "REPRODUCIENDO" : "EN PAUSA"}
                                </strong>
                                <span>{this.state.currentTime} / {this.state.totalDuration}</span>
                            </p>
                            <button
                                className="heart-icon"
                                dangerouslySetInnerHTML={{__html: this.state.favourite ? `<svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 47 45" style="enable-background:new 0 0 47 45;" xml:space="preserve"><path d="M46.8,14.8c0,16-21.3,29.1-22.2,29.7c-0.3,0.2-0.7,0.3-1,0.3c-0.4,0-0.7-0.1-1-0.3C21.6,43.9,0.2,30.8,0.2,14.8 c0-8.3,6-14.6,13.9-14.6c2.6,0,5.1,0.7,7.3,2c0.7,0.4,1.5,0.9,2.1,1.5c0.7-0.6,1.4-1,2.1-1.5c2.2-1.3,4.7-2,7.3-2 C40.8,0.2,46.8,6.5,46.8,14.8z"/></svg>`
                        ...
                        <div className="volume-bar-ctr" id="volumeBarCtr">
                            ...
                            >
                                <button
                                    className="handle"
                                    style={{
                                        background: this.state.volumeHandleActive ? "#1DB954" : null
                                    }}
                                ></button>
                            </div>
                        </div>
